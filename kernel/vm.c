...
kvmcreate()
{
  ...
  // just copy #1-512 ptes from kernel_pagetable.
  for(int i=1;i<512;i++)
...
void printwalk(pagetable_t pagetable, uint16 level){
  for(int i=0;i<512;i++){
    pte_t pte=pagetable[i];
    if(pte&PTE_V){
      uint64 child=PTE2PA(pte);
...
int
kvmmapuser(pagetable_t pagetable, pagetable_t kpagetable, uint64 oldsz, uint64 newsz)
{
  uint64 va;
  pte_t *upte;
  pte_t *kpte;
  ...
  if(newsz>PLIC){
    panic("kvmmapuser: out of range plic");
    return -1;
  }
  ...
  for(va=oldsz;va<newsz;va+=PGSIZE){
    ...
    if(upte==0)
      panic("kvmmapuser: upte==0");
    if((*upte & PTE_V)==0)
      panic("kvmmapuser: invalid pte");
    kpte=walk(kpagetable,va,1);
    if(kpte==0)
      panic("kvmmapuser: kpte==0");
    *kpte=*upte;
    *kpte &= ~PTE_U;
  }
